...
class Instruction (object):
    ...
    def _toText(self):
        ...
        text = Decode(self.address, self.instructionBytes, self.dt)[0][2]
        hexStr = Decode(self.address, self.instructionBytes, self.dt)[0][3]
        return text +" "+ hexStr
...
def DecomposeGenerator(codeOffset, code, dt, features = 0):
    ...
    while codeLen > 0:
        ...
        usedInstructionsCount = c_uint(0)
        codeInfo = _CodeInfo(_OffsetType(codeOffset), _OffsetType(0), cast(p_code, c_char_p), codeLen, dt, c_uint(features))
